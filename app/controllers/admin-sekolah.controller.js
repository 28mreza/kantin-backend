...
exports.findAll = async (req, res) => {
    try {
        const data = await User.find({ roles: 'adminsekolah', isDeleted: 0 }).sort({ _id: -1 });
        res.status(200).send({
            status: true,
...
exports.show = async (req, res) => {
    ...
    try {
        const data = await User.findOne({ _id: id, roles: 'adminsekolah' });
        if (!data) {
...
exports.create = async (req, res) => {
    ...
    const password = bcrypt.hashSync('adminsekolah', 8);
    const fotoProfil = req.file.path;
    const roles = 'adminsekolah';
    const status = 1;
